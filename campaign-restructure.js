...
// Step 1: Get existing campaign configuration
async function getCampaignConfig(campaignId) {
  await log(`Fetching configuration for campaign ${campaignId}`);
  
  try {
    // Get basic campaign info from campaign stats
    const stats = await apiRequest('/campaign/get/stats', 'GET', null, { 
      workspace_id: config.workspaceId,
      campaign_id: campaignId 
    });
    
    await log(`Campaign ${campaignId} config fetched successfully`);
    return stats;
  } catch (error) {
    await log(`Error fetching campaign config: ${error.message}`);
...
// Step 2: Create new West campaign
...
// Step 3: Get NOT_CONTACTED leads from campaign
...
// Step 4: Separate leads by timezone
...
// Step 5: Delete leads from campaign
async function deleteLeadsFromCampaign(campaignId, leadEmails) {
  ...
  for (let i = 0; i < leadEmails.length; i += batchSize) {
    ...
    try {
      ...
      try {
        await apiRequest('/lead/delete', 'DELETE', {
          workspace_id: config.workspaceId,
          campaign_id: campaignId,
          email: batch
        });
...
// Step 6: Add leads to campaign from CSV
...
// Step 7: Get all email accounts
async function getEmailAccounts() {
  await log('Fetching all email accounts in workspace');
  
  try {
    const response = await apiRequest('/email-account/list', 'GET', null, {
      workspace_id: config.workspaceId
    });
    
    const accountIds = response.email_accounts ? response.email_accounts.map(acc => acc.id) : [];
    await log(`Found ${accountIds.length} email accounts`);
    return accountIds;
  } catch (error) {
    await log(`Error fetching accounts: ${error.message}`);
    return [];
  }
}

// Step 8: Assign email accounts to campaign
async function assignAccountsToCampaign(campaignId, accountIds) {
  if (accountIds.length === 0) return;
  
  await log(`Assigning ${accountIds.length} accounts to campaign ${campaignId}`);
  
  try {
    await apiRequest('/campaign/update/campaign', 'PATCH', {
      workspace_id: config.workspaceId,
      campaign_id: campaignId,
      email_accounts: accountIds
    });
    await log(`Successfully assigned accounts to campaign ${campaignId}`);
  } catch (error) {
    await log(`Error assigning accounts: ${error.message}`);
  }
}

// Step 9: Rename campaign
...
async function processCampaign(campaign) {
  ...
  try {
    ...
    await log('\n--- Step 1: Getting campaign configuration ---');
    const campaignConfig = await getCampaignConfig(campaign.id);
    
    // Step 2: Create West version
    await log('\n--- Step 2: Creating West campaign ---');
    const westCampaignId = await createWestCampaign(campaign.name);
    
    // Step 3: Get NOT_CONTACTED leads
    await log('\n--- Step 3: Fetching NOT_CONTACTED leads ---');
    const notContactedLeads = await getNotContactedLeads(campaign.id);
    
    // Step 4: Separate by timezone
    await log('\n--- Step 4: Separating leads by timezone ---');
    ...
    // Step 5: Move West leads
    await log('\n--- Step 5: Moving West leads ---');
    ...
    // Step 6: Get and assign email accounts
    await log('\n--- Step 6: Assigning email accounts ---');
    const emailAccounts = await getEmailAccounts();
    if (emailAccounts.length > 0) {
      await assignAccountsToCampaign(campaign.id, emailAccounts); // East
      await assignAccountsToCampaign(westCampaignId, emailAccounts); // West
    }
    
    // Step 7: Rename East campaign
    await log('\n--- Step 7: Renaming East campaign ---');
...
async function main() {
  try {
    await log('='.repeat(80));
    await log('Starting PlusVibe Campaign Restructure');
    await log(`Processing ${campaigns.length} campaigns into East/West versions`);
    await log('='.repeat(80));
    
    const results = [];
    
    // Test with Retail campaign first (smallest)
    const testCampaign = campaigns.find(c => c.name.includes('Retail'));
    await log(`\n*** TESTING WITH: ${testCampaign.name} ***\n`);
    
    const testResult = await processCampaign(testCampaign);
    results.push(testResult);
    
    await log('\n*** TEST COMPLETED SUCCESSFULLY ***');
    await log('*** The test campaign has been processed. ***');
    await log('*** To process all campaigns, uncomment the loop below. ***\n');
    
    // Uncomment this to process ALL campaigns after testing
    /*
    for (const campaign of campaigns) {
      if (campaign.name.includes('Retail')) continue; // Skip test campaign
