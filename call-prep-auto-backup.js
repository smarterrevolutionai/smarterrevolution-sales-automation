...
n// CLI argument parsing - must be at the top
...
if (showHelp) {
  console.log(`
Discovery Call Prep Automation v1.0

USAGE:
  node call-prep-auto.js [OPTIONS]

OPTIONS:
  -h, --help     Show this help message
  -d, --dry-run  Test run without sending SMS (shows what would be sent)

DESCRIPTION:
  Automatically checks for upcoming discovery calls in the next 30 minutes
  and sends preparation briefs via SMS to Wolf (+12133028260).
  
  Runs every 15 minutes via cron job to ensure timely delivery.
  
EXAMPLES:
  node call-prep-auto.js           # Normal execution
  node call-prep-auto.js --dry-run # Test without sending SMS
  node call-prep-auto.js --help    # Show this help

LOGS:
  /var/log/call-prep-auto.log     # Cron job logs
  /opt/smarty-projects/call-prep-auto.log # Application logs
  
`);
  process.exit(0);
}

// CLI execution with dry-run support
if (require.main === module) {
  if (isDryRun) {
    console.log('🧪 DRY RUN MODE - No SMS will be sent');
    // Mock the sendSMS function for dry run
    const originalSendSMS = module.exports.sendSMS;
